          ...
          <div className="lg:col-span-3 grid grid-cols-12 gap-3 rounded-2xl bg-[#0f0f0f] p-4 ring-1 ring-white/10">
            ...
            {coins.map((c) => (
              <div
                ...
                style={{
                  gridColumn: `span ${Math.max(2, Math.round((c.share / 10) * 3))} / span ${Math.max(2, Math.round((c.share / 10) * 3))}`,
                }}
              >
                ...
                {[
                  ...
                  <tr key={r.code} className="hover:bg-white/5">
                    <td className="px-4 py-3 text-white/90">
                      {r.name} <span className="text-white/50">({r.code})</span>
                    </td>
                    <td className="px-4 py-3">{r.price}</td>
                    <td
                      className={`px-4 py-3 ${r.change.startsWith("-") ? "text-red-400" : "text-emerald-400"}`}
                    >
                      {r.change}
                    </td>
                    ...
                    <td className="px-4 py-3">
                      <a
                        className="rounded-full bg-yellow-300 px-3 py-1 text-xs font-semibold text-black"
                        href={`/chart/${r.code}`}
                      >
                        View
                      </a>
